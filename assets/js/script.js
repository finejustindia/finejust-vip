...
(function() {
  ...
  // Central Data Layer Integration
  async function getToolsData() {
    if (window.__FINEJUST_DATA__ && window.__FINEJUST_DATA__.tools && window.__FINEJUST_DATA__.tools.length > 0) {
      return window.__FINEJUST_DATA__.tools;
    }
    try {
      const res = await fetch(`${rootPrefix}assets/data/tools.json`);
      if (res.ok) {
        const data = await res.json();
        window.__FINEJUST_DATA__ = window.__FINEJUST_DATA__ || {};
        window.__FINEJUST_DATA__.tools = data;
        return data;
      }
    } catch (e) {}
    return [];
            ...
            <div class="nav-dropdown-wrapper">
              ...
              <div class="nav-dropdown-menu">
                ...
                <a href="${rootPrefix}tools.html" class="dropdown-item-all">
                  <span>Explore All 30+ Free Tools</span>
                  <span>&rarr;</span>
                </a>
              </div>
            </div>
          ...
          <div class="footer-grid-5">
            ...
            <div>
              ...
              <ul class="footer-links">
                ...
                <li><a href="${rootPrefix}tools.html" class="footer-link" style="color:var(--primary); font-weight:700;">All 30+ Free Tools &rarr;</a></li>
              </ul>
            </div>
  ...
  // Dynamic Related Tools Generator (Loads 6 Related Tools from Data Layer)
  async function initDynamicRelatedTools() {
    ...
    let related = allTools;

    if (currentCategory !== 'all') {
      const matchCat = related.filter(t => (t.category || '').toLowerCase() === currentCategory.toLowerCase());
    ...
    selected.forEach(tool => {
      ...
      html += `
        <a href="${rootPrefix}${targetUrl}" class="tool-card" style="padding:1.25rem; border-radius:var(--radius-md);">
  ...
  async function initRecentlyViewedTools() {
    const path = window.location.pathname.replace(/\\/g, '/').toLowerCase();
    const allTools = await getToolsData();
    ...
    if (path.includes('/tools/')) {
      ...
      const currentTool = allTools.find(t => (t.url && t.url.endsWith(filename)) || (t.slug && filename.includes(t.slug)));
      
      if (currentTool) {
        try {
          let recent = JSON.parse(localStorage.getItem('finejust_recent_tools') || '[]');
          recent = recent.filter(item => item.id !== currentTool.id && item.slug !== currentTool.slug);
          recent.unshift({
            id: currentTool.id,
        ...
        <div class="content-box" style="margin-top: 2.5rem; margin-bottom: 2.5rem;">
          ...
          <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(240px, 1fr)); gap:1rem;">
            ${recent.map(item => {
              ...
              return `
              <a href="${prefix}${url}" class="content-box" style="margin:0; padding:1.15rem; display:flex; align-items:center; gap:0.85rem; border-color:var(--border);">
  ...
  async function initPopularTools() {
      ...
      c.innerHTML = `
        ...
        <div class="tools-grid">
          ${tools.map(t => {
            ...
            if (url.startsWith('/')) url = url.substring(1);
            return `
            <a href="${prefix}${url}" class="tool-card">
